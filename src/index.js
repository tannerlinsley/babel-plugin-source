export default function sourcePlugin(babel) {
  const { types: t } = babel
  ...
  return {
    ...
    visitor: {
      Scopable(path) {
        ...
        path.traverse(expressionVisitor, { handleComment })
        ...
        Object.keys(sources).forEach(sourceName => {
          ...
          const variablePath = path.find(
            path => path.scope.bindings[sourceName]
          )
          ...
          variablePath.scope.bindings[
            sourceName
          ].path.node.init = t.stringLiteral(sourceCode)
...
function makeSourceCode(source, startComment, endComment) {
  // console.log(startComment, endComment)
  ...
  ].slice(0, isSingleLine ? endColumn - startColumn : endColumn)
